...
allplants<-allplant%>%left_join(biomass.plant, by=c('species','sample'='Sample', 'treat'))%>%
  mutate_at(vars(plant_total_mg:pred_abun), funs(dens=./plant_g))
allplants
...
lrr.sps<-lrr.all%>%dplyr::select(species,variable, yi, sei)%>%
...
allplants_mean<-allplants%>%
  ...
  left_join(sps.means, by='species')%>%
...
Test for effect of plant species * treatment  

```{r}
str(allplants_mean)
plot(allplants_mean%>%
       dplyr::select(stem_mean, resist = T_herb_mg_dens_mean, 
                     lrr_herb = herb_mg_dens_yi, lrr_pred = pred_mg_dens_yi,
                     iph_t = T_iph_mg_omni_mean, iph_c = C_iph_mg_omni_mean, 
